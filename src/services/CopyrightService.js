...
class CopyrightService {
    ...
    async submitCopyright(manuscriptId, templateVersion, signatures) {
        ...
        const template = await copyrightRepository.findTemplateByVersion(templateVersion);
        if (!template) {
            throw new Error(`Copyright template version "${templateVersion}" not found`);
        }

        // Check if submission already exists
        const existingSubmission = await copyrightRepository.findSubmissionByManuscriptId(manuscriptId);
        if (existingSubmission) {
            throw new Error('Copyright agreement already submitted for this manuscript');
        }

        // Create the copyright submission (no manuscript status update)
        const result = await copyrightRepository.createSubmission({
    ...
    async getSubmissionByManuscriptId(manuscriptId) {
        const submission = await copyrightRepository.findSubmissionByManuscriptId(manuscriptId);
    ...
    async getManuscriptForCopyright(manuscriptId) {
        const manuscript = await copyrightRepository.findManuscriptWithAuthors(manuscriptId);
