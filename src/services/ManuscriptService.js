...
class ManuscriptService {
   ...
   * @param {boolean} emailTrigger - Whether to send emails (from user token)
   */
  async submitManuscript(data, files, emailTrigger = true) {
    // Validation: Check if journal exists
    const journal = await journalRepository.findById(
      data.journalId || data.journal,
    );
    if (!journal) {
    ...
    // Map files
    const manuscriptFile = files["manuscriptFile"]
      ? files["manuscriptFile"][0]
      : null;
    const coverLetterFile = files["coverLetter"]
      ? files["coverLetter"][0]
      : null;
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      const customManuscriptId =
        await manuscriptRepository.generateManuscriptId(journal.id);
      ...
      const manuscriptData = {
        ...
        reviewer_specialisation:
          data.reviewerSpecialisation || data.revSpecialisation,
      ...
      if (data.checklist) {
        const checklistData = {
          ...
          has_acknowledged_support:
            data.checklist.hasAcknowledgedSupport || false,
          has_acknowledged_funding:
            data.checklist.hasAcknowledgedFunding || false,
