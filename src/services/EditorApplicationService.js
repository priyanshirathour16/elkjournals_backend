...
class EditorApplicationService {
  async submitApplication(data) {
    ...
    const existingApplication = await editorApplicationRepository.findByEmail(
      data.email,
    );
    ...
    if (existingApplication) {
      ...
      if (
        ...
        // Update fields
        await editorApplicationRepository.update(
          existingApplication.id,
          applicationData,
        );
        resultApplication = await editorApplicationRepository.findById(
          existingApplication.id,
        );
      } else {
      ...
      // New application
      resultApplication =
        await editorApplicationRepository.create(applicationData);
    }

    // Send welcome email with credentials (always send on creation - don't check email_trigger)
    try {
